...
def create_config country
  vpn_host = "#{country.code}.#{ENV.fetch('OPENVPN_HOST', Rails.application.credentials.vpn_host)}".downcase
...
[
  ...
  path_to_file = Rails.root.join('app', 'assets', 'images', 'icons', 'app_clients', 'pink', 'svg', filename)
...
[
  ...
  User.all.each do |user|
    ...
    user.email_subscriptions << item if !user.email_subscriptions.include?(item)
    user.save
  end
end